...
export default function AdminAcceso() {
  ...
  const revocarAcceso = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/actores/${id}/desactivar`, {
        method: "PATCH",
        headers: {
            ...
            <div className="flex">
                <div className="ml-3">
                    ...
                    <p className="text-sm text-yellow-700 mt-1">
                        Está modificando el estado activo de un Actor con Derecho, los programas y planes que ejecutan tareas de restitución de derechos sobr esta entidad ya no estaran disponibles si desea continuar
                    </p>
                </div>
            </div>
        </div>
